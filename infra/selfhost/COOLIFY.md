...
The first deploy pulls ten images, about 2 GB. Nothing is built.

When it finishes there are **thirteen containers, and three of them have
exited**. That is success, not failure: `migrate`, `keygen` and `tracker-build`
are one-shot steps that do their work and stop. The other ten stay up and go
healthy, `web` last.
