    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Wraps rather than truncates. `truncate` carries
              `white-space: nowrap`, which makes the *whole accordion's*
              min-content width the longest title — and in a grid cell with an
              `auto` track, that minimum widens the track past its container
              and the rows hang off the right edge of a phone. It also cut the
              longest question mid-word, which is the wrong trade for a
              question. Two lines still fit the 54px trigger, so nothing that
              depends on the row height changes. */}
          <span
            className={cn(
